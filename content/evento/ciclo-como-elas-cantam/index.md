---
date: 2019-02-15T23:33:21Z
description: ""
draft: false
resources:
- src: images/bogomil-mihaylov-519207-unsplash.jpg
  name: "header"
- src: "gallery/*.jpg"
  name: gallery-:counter
  title: gallery-title-:counter
slug:
subtitle: 

tags:
  - 
categorias:
  - Cinema
ciclo: Como Elas Cantam

title: 'Ciclo “como Elas Cantam!”'

options:
  hideFooter: false
  hideSubscribeForm: false
  showHeader: true
  unlisted: false
scripts:
  -
weight: 20190403
custom:
  copyright: 'Bogomil Mihaylov no Unsplash'
  data: 2019-04-03T17:35:00Z
  local: Salão Brazil
---

Três figuras maiores da música, retratadas com mestria e paixão: **Janis Joplin**, considerada por muitos a maior cantora de blues e soul da sua geração, influenciada por grandes nomes do jazz e blues; **Sarah Vaughan**, justamente eleita como uma das três divas do jazz, protagonista de uma carreira profissional admirável; **Violeta Parra**, destacada compositora e cantora chilena, heroína nacional, tendo alcançado reconhecimento e prestígio internacionais. 

*Vida e obra, entre memórias e canções, num registo emocional e biográfico.* 

Salão Brazil    
Sessões às 21h30    
Entrada: 2,00 €    
Sócios JACC e Fila K: 1,00 €    
Bilhetes à venda no local, no próprio dia, a partir das 21h00    

<div class="row gtr-50 gtr-uniform">

<div class="col-4">
  <p>3 de Abril</p>
<h3>“Janis: Little Girl Blue”</h3>
  <p><strong>De</strong> <a href='http://us.imdb.com/Name?Amy%20Berg'><strong>Amy Berg</strong></a></p>
  <p><strong>Documentário, Musical</strong></p>
  <p><strong>Apresentação de Francisco Amaral</strong></p>
</div>

<div class="col-4">
  <p>11 de Abril</p>
  <h3>“Sarah Vaughan: The Divine One”</h3>
  <p><strong>De Matthew Seig</strong></p>
  <p><strong>Documentário, Musical</strong></p>
  <p><strong>Apresentação de Rui Moutinho</strong></p>
</div>
<div class="col-4">
  <p>17 de Abril</p>
  <h3>“Violeta foi para o Céu”</h3>
  <p><strong>De Andrés Wood</strong></p>
  <p><strong>Biografia</strong></p>
  <p><strong>Apresentação de Né Ladeiras</strong></p>
</div>
</div>




<!--more-->

Três figuras maiores da música, retratadas com mestria e paixão: **Janis Joplin**, considerada por muitos a maior cantora de blues e soul da sua geração, influenciada por grandes nomes do jazz e blues; **Sarah Vaughan**, justamente eleita como uma das três divas do jazz, protagonista de uma carreira profissional admirável; **Violeta Parra**, destacada compositora e cantora chilena, heroína nacional, tendo alcançado reconhecimento e prestígio internacionais. 

*Vida e obra, entre memórias e canções, num registo emocional e biográfico.* 

Salão Brazil    
Sessões às 21h30    
Entrada: 2,00 €    
Sócios JACC e Fila K: 1,00 €    
Bilhetes à venda no local, no próprio dia, a partir das 21h00    
<div class="bypass">
  <div class="row gtr-uniform">
  <div class="col-4">
    <h2>3 de Abril</h2>
  <h3>“Janis: Little Girl Blue”</h3>
    <p>De <a href='http://us.imdb.com/Name?Amy%20Berg'>Amy Berg</a></p>
    <p>Documentário, Musical</p>
    <p>Apresentação de Francisco Amaral</p>
  </div>  
  <div class="col-4">
    <h2>11 de Abril</h2>
    <h3>“Sarah Vaughan: The Divine One”</h3>
    <p>De Matthew Seig</p>
    <p>Documentário, Musical</p>
    <p>Apresentação de Rui Moutinho</p>
  </div>  
  <div class="col-4">
    <h2>17 de Abril</h2>
    <h3>“Violeta foi para o Céu”</h3>
    <p>De Andrés Wood</p>
    <p>Biografia</p>
    <p>Apresentação de Né Ladeiras</p>
  </div>
  </div>
</div>

<style>.image.main{display:none;}</style>